...
def file_block(src_name, pathurl, tb, ntsc, src_frames, w, h, sr, ch, indent, video=True):
    pad = " " * indent
...
{pad}    </video>
""" if video else ""
    # The file's declared channel count MUST match the real media or Premiere
    # refuses to relink ("it has N audio channel(s) and the clip was created
    # with M ... with a different channel type"). Undeclared => Premiere assumes mono.
...
{pad}      </samplecharacteristics>
{pad}      <channelcount>{ch}</channelcount>
{pad}    </audio>
{pad}  </media>
{pad}</file>"""
...
{common}
{file_block(src_name, pathurl, tb, ntsc, src_frames, w, h, sr, ch, 12)}
            <sourcetrack>
              <mediatype>video</mediatype>
              <trackindex>1</trackindex>
            </sourcetrack>
...
{common}
{file_block(src_name, pathurl, tb, ntsc, src_frames, w, h, sr, ch, 12, video=False)}
            <sourcetrack>
              <mediatype>audio</mediatype>
              <trackindex>1</trackindex>
            </sourcetrack>
...
"""
    out = spec["output"]
    # the 'output' field is the XML TIMELINE path. Refusing anything else
    # is load-bearing: pointing it at a render once destroyed two finished
    # MP4s (field report 2026-08-22) — a wrong field must never be able to
    # overwrite media.
    if os.path.splitext(out)[1].lower() != ".xml":
        sys.exit(f"refusing to write XML to '{out}' — the cuts JSON "
                 "'output' field must be the .xml timeline path, NOT the "
                 "clip MP4. Set it to e.g. 'clip 01 timeline.xml' and "
                 "re-run.")
